...
def registerBulkProduct(level1Category, fileName):
    try:
        ...
        while i < len(records) and i < 1098:
            product = {'Level1 Category': records[i][1], 'Main Category': records[i][2], 'Sub Category': records[i][3], 'product_name': records[i][5],
                       'Product Category': records[i][4], 'Quantity': [{'City': 'Hyderabad', 'Quantities': []}],
                       '_id': '1_' + str(MainCategories.index(records[i][2])) + '_' + str(
                           categories[MainCategories.index(records[i][2])][records[i][2]].index(records[i][3]))}
            j = i
...
>>>>>>> 0ce51dba9c38e51695ceb9e382f300883b36ebdd
