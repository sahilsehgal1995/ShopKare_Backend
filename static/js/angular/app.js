var app = angular.module('Shopkare', ['ui.router']);
...
$scope.level3Categories=[];   
  ...
  .error(function(err){
      ...
      $scope.logOUT=function(){
        ...
        var z=[ id.images , id.product_name , id.Qwt[k] , id.Qrs[k], id.description ]; 
...
app.controller('checkout',['$scope','$http','$window',function($scope,$http,$window){
      ...
      var storedData =localStorage.getItem("product");
      $scope.order.CartProducts.push(storedData);
        if($scope.order.CartProducts){
          console.log($scope.order.CartProducts);
          $scope.messageEmptyCart=false;
        }
        else{
